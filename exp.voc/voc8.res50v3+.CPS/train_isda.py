...
with Engine(custom_parser=parser) as engine:
    ...
    print(f"exp_name:{config.exp_name}, exp_num:{config.exp_num}, labeled_ratio:{config.labeled_ratio}, nepochs:{config.nepochs} ")
    ...
    for epoch in range(engine.state.epoch, config.nepochs):
        ...
        for idx in pbar:
            ...
            if config.exp_num == -1:
                ...
                # print(gts.shape)
                _, h, w = max_l.shape
                x_isda_pred_unsup_l = F.interpolate(input=x_isda_unsup_l, size=(h, w), mode='bilinear', align_corners=True)
                loss_isda_unlabeled = criterion(x_isda_pred_unsup_l, max_r)

                x_isda_pred_unsup_r = F.interpolate(input=x_isda_unsup_r, size=(h, w), mode='bilinear', align_corners=True)
                loss_isda_unlabeled += criterion(x_isda_pred_unsup_r, max_l)

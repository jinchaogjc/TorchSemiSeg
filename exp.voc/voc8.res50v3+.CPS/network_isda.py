...
class Network(nn.Module):
    ...
    def forward(self, data, step=1, isda=False):
        if not self.training:
            pred1 = self.branch1(data)
            return pred1

        if step == 1:
            return self.branch1(data, isda=isda)
        elif step == 2:
            return self.branch2(data, isda=isda)
...
class SingleNetwork(nn.Module):
    def __init__(self, num_classes, criterion, norm_layer, pretrained_model=None):
        ...
        #     BatchNorm2d(512), nn.LeakyReLU(negative_slope=1e-2, inplace=True),
        #     nn.Dropout2d(0.1),
        # )
        self.dsn = nn.Sequential(
            nn.Conv2d(1024, 256, kernel_size=3, stride=1, padding=1),
            BatchNorm2d(256), nn.LeakyReLU(negative_slope=1e-2, inplace=True),
            nn.Dropout2d(0.1),
        )
        ...
        self.final_conv_1 = nn.Conv2d(256, num_classes, kernel_size=1, stride=1, padding=0, bias=True)
        self.business_layer.append(self.final_conv_1)
        # self.final_conv_2 = nn.Conv2d(256, num_classes, kernel_size=1, stride=1, padding=0, bias=True)
    ...
    def forward(self, data, isda=False):
        ...
        x = self.final_conv_1(v3plus_feature)
        # x_dsn = self.final_conv_2(feature_x_dsn)
        ...
        if isda:
            # return [x, x_dsn], [v3plus_feature.detach(), feature_x_dsn.detach()], v3plus_feature, pred
            # return x, x_dsn, feature_x_dsn.detach(), v3plus_feature, pred
            return x, v3plus_feature, pred
        if self.training:
            return v3plus_feature, pred
        return pred
...
if __name__ == '__main__':
    ...
    model = Network(num_classes=21, criterion=nn.CrossEntropyLoss(),
                    pretrained_model=None,
                    norm_layer=nn.BatchNorm2d)
    print(model.branch1.training)
    image = torch.randn(2, 3, 512, 512)
    # [x, x_dsn], [v3plus_feature.detach(), feature_x_dsn.detach()], v3plus_feature, pred
    # x_list, feature_x_list, v3plus_feature, pred = model(image, isda=True)
    # x, x_dsn = x_list[0], x_list[1]
